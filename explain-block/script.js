  ...
  async function ensureSettingsPage(openInSidebar = true) {
    ...
    if (!headerRows?.[0]?.[0]) {
      ...
      await window.roamAlphaAPI.data.block.create({
        ...
        block: { uid: headerUid, string: "**How to use this page** — every setting below is `key:: value`. Edit inline. Reload via cmd palette → \"Explain block: reload settings from graph\"." },
      });
    }
    let order = 1;
